...
void function FSV_SkipThread()
{
	...
	{
		FSU_ChatBroadcast(
			"%H" + FSV_TimerToMinutesAndSeconds( timer ) + "%N - " + "A vote to skip this map has been started! Use %H%Pskip %Nto vote. %T" + skipThreshold +
	...
	while ( timer > 0 && skipVoters.len() < skipThreshold )
	{
		if ( timer == nextUpdate )
		{
			...
			{
				FSU_ChatBroadcast(
					"%H" + FSV_TimerToMinutesAndSeconds( timer ) + "%H - " + skipVoters.len() + "/" + skipThreshold +
						"%N have voted to skip this map. %TUse %H%Pskip %Tto vote."
				)
...
void function FSV_ExtendThread()
{
	...
	if ( GetConVarBool( "FSV_ENABLE_RUI" ) )
	{
		foreach ( entity player in GetPlayerArray() )
		{
			NSSendAnnouncementMessageToPlayer(
				player,
				"VOTE TO EXTEND MAP TIME STARTED",
				"Use '" + GetConVarString( "FSCC_PREFIX" ) + "extend' in chat to add your vote",
				< 1, 0, 0 >,
				0,
				1
			)
		...
		foreach ( entity player in GetPlayerArray() )
		{
			NSCreateStatusMessageOnPlayer(
				player,
				FSV_TimerToMinutesAndSeconds( timer ),
				extendVoters.len() + "/" + extendThreshold + "have voted to extend map time",
				"extend"
			)
		}
	}
	...
	{
		FSU_ChatBroadcast(
			"%H" + FSV_TimerToMinutesAndSeconds( timer ) + "%N - " + "A vote to extend map time has been started! Use %H%Pextend %Nto vote. %T" + extendThreshold +
				" votes will be needed."
		)
	}
	...
	while ( timer > 0 && extendVoters.len() < extendThreshold )
	{
		if ( timer == nextUpdate )
		{
			...
			{
				FSU_ChatBroadcast(
					"%H" + FSV_TimerToMinutesAndSeconds( timer ) + "%N - %H " + extendVoters.len() + "/" + extendThreshold +
						"%N have voted to extend map time. %TUse %H%Pextend %Tto vote."
				)
...
void function FSV_CommandCallback_Kick( entity player, array<string> args )
{
	...
	foreach ( string uid in playersWithActiveVotes )
	{
		...
		{
			FSU_PrivateChatMessage(
				player,
				"%EYou can only have one kickvote active at a time! %TWait for the current one to expire or succeed before starting another."
			)
			return
		}
	}
...
void function FSV_KickThread( string targetName, string targetUid )
{
	...
	if ( GetConVarBool( "FSV_ENABLE_RUI" ) )
	{
		foreach ( entity player in GetPlayerArray() )
		{
			if ( player.GetUID() != targetUid )
			{
				NSSendAnnouncementMessageToPlayer(
					player,
					"VOTE TO KICK " + targetName + " STARTED",
					"Use '" + GetConVarString( "FSCC_PREFIX" ) + "kick <part-of-their-name>' in chat to contribute your vote.",
					< 1, 0, 0 >,
					0,
					1
				)
		...
		foreach ( entity player in GetPlayerArray() )
		{
			if ( player.GetUID() != targetUid )
			{
				NSCreateStatusMessageOnPlayer(
					player,
					FSV_TimerToMinutesAndSeconds( timer ),
					kickTable[ targetUid ].voters.len() + "/" + kickTable[ targetUid ].threshold + " have voted to kick " + targetName,
					"kick" + targetName
				)
			}
		}
	...
	while ( timer > 0 && kickTable[ targetUid ].voters.len() < kickTable[ targetUid ].threshold )
	{
		if ( timer == nextUpdate )
		{
			...
			{
				FSU_ChatBroadcast(
					"%H" + FSV_TimerToMinutesAndSeconds( timer ) + "%N - %H " + kickTable[ targetUid ].voters.len() + "/" + kickTable[ targetUid ].threshold +
						"%N have voted to kick %H" + targetName + "%N. %TUse %H%Pkick  " + targetName + "%T to vote."
				)
...
//
// 	 foreach (entity player in GetPlayerArray()) {
// 		 NSSendAnnouncementMessageToPlayer( player, "VOTE TO TEST STARTED", "Use
// '"+GetConVarString( "FSCC_PREFIX" )+"test' in chat to add your vote.", <1,0,0>, 0, 1 )
// 	 }
// 	 wait 1
// 	 foreach (entity player in GetPlayerArray()) {
...
// 				 FSU_ChatBroadcast( "%F[%T"+FSV_TimerToMinutesAndSeconds(timer)+"
// %FREMAINING]%H "+ testVoters + "/" + threshold + "%N have voted to test. %TUse
// %H%Ptv %Tto vote." )
